...
if not st.session_state.token and not st.session_state.offline_mode:
    ...
    render_main_workspace()
        ...
        with tab_products: # Продукты
            st.subheader("Библиотека Продуктов (ПТУ)")
            st.info("Управляйте жизненным циклом ваших продуктов. Перемещайте их по стадиям от идеи до активного актива.")
            ...
            # --- ШАГ 12: РЕАЛЬНАЯ КАНБАН-ДОСКА НА ОСНОВЕ API ---
            p_col1, p_col2, p_col3, p_col4 = st.columns(4)
            
            columns = {
                "Idea": p_col1,
                "In Development": p_col2,
                "Active": p_col3,
                "Archived": p_col4
            }
            ...
            for status, col in columns.items():
                with col:
                    st.markdown(f"##### {column_titles[status]}")
                    # Фильтруем продукты для текущей колонки
                    products_in_column = [p for p in st.session_state.products if p.get('status') == status]
                    for product in products_in_column:
                        with st.container(border=True):
                            ...
                            with st.expander("Редактировать"):
                                new_name = st.text_input("Название", value=product.get('name'), key=f"name_{product['product_id']}")
                                new_price = st.number_input("Цена", value=float(product.get('price', 0.0)), key=f"price_{product['product_id']}", format="%.2f")
                                new_status = st.selectbox("Статус", options=column_titles.keys(), index=list(column_titles.keys()).index(product.get('status')), key=f"status_{product['product_id']}")
                                
                                if st.button("Сохранить", key=f"save_{product['product_id']}"):
                                    if not st.session_state.offline_mode:
                                        api_url = f"http://127.0.0.1:8000/api/v1/products/{product['product_id']}"
                                        headers = {"Authorization": f"Bearer {st.session_state.token}"}
                                        payload = {"name": new_name, "price": new_price, "status": new_status}
                                        
                                        response = requests.put(api_url, headers=headers, json=payload)
                                        if response.status_code == 200:
                                            # Обновляем данные в session_state
                                            product.update(response.json())
                                            st.toast("✅ Продукт обновлен!")
                                            st.rerun()
                                        else:
                                            st.error(f"Ошибка: {response.text}")
                                    else:
            ...
            with st.expander("Почему именно такая продуктовая линейка?"):
                ...
                with st.expander("Показать детальный расчет"):
                    st.write(f"Посетители: {int(traffic):,}") # type: ignore
                    st.write(f"Лиды (C1 = {c1:.1%}): {int(leads):,}") # type: ignore
                    st.write(f"Покупатели Трипвайера (C2 = {c2:.1%}): {int(tripwire_buyers):,}") # type: ignore
                    st.write(f"Доход с Трипвайера: ${int(tripwire_revenue):,}") # type: ignore
                    st.write(f"Покупатели Core Offer (C3 = {c3:.1%}): {int(core_offer_buyers):,}") # type: ignore
                    st.write(f"Доход с Core Offer: ${int(core_offer_revenue):,}") # type: ignore
                    st.write(f"**Итоговый доход:** **${int(total_revenue):,}**") # type: ignore
        ...
        with tab_tasks: # Задачи
            ...
            # --- ШАГ 23: ФИЛЬТРАЦИЯ ПО ТЕГАМ (ЗАМЫКАНИЕ ЦИКЛА RMS) ---
            all_tags = sorted(list(set(tag for member in st.session_state.team_members for tag in member.get('tags', []))))
            
            if all_tags:
                selected_tags = st.multiselect(
                    "Фильтр по навыкам (тегам):",
                    options=all_tags,
                    help="Показывает задачи, назначенные на исполнителей с выбранными тегами, а также нераспределенные задачи."
                )
            else:
                selected_tags = []
            ...
            if selected_tags:
                # Находим ID исполнителей с нужными тегами
                responsible_ids_with_tags = {
                    member['member_id'] for member in st.session_state.team_members 
                    if any(tag in member.get('tags', []) for tag in selected_tags)
                }
                # Фильтруем задачи: показываем либо нераспределенные, либо те, что на нужных исполнителях
                tasks_to_display = [
                    task for task in st.session_state.tasks 
                    if not task.get('responsible_id') or task.get('responsible_id') in responsible_ids_with_tags
                ]
            # --- НОВЫЙ БЛОК: ВЫБОР СЦЕНАРИЯ ДЛЯ ДЕКОМПОЗИЦИИ ---
        ...
        with tab_messenger:
            st.subheader("💬 CorePath Мессенджер")
            st.info("Защищенное пространство для коммуникации с вашей командой и продюсером.")

            msg_col1, msg_col2 = st.columns([1, 3])

            with msg_col1:
                st.markdown("#### Каналы")
                if not st.session_state.channels:
                    st.info("Каналы не найдены.")
                    if st.button("Создать 'Основной' канал"):
                        if st.session_state.client_profile and not st.session_state.offline_mode:
                            profile_id = st.session_state.client_profile.profile_id
                            api_url = f"http://127.0.0.1:8000/api/v1/profiles/{profile_id}/channels"
                            headers = {"Authorization": f"Bearer {st.session_state.token}"}
                            payload = {"channel_name": "Основной"}
                            response = requests.post(api_url, headers=headers, json=payload)
                            if response.status_code == 201:
                                st.session_state.channels.append(response.json())
                                st.rerun()
                else:
                    for channel in st.session_state.channels:
                        if st.button(channel['channel_name'], key=channel['channel_id'], use_container_width=True, type="secondary" if st.session_state.selected_channel_id != channel['channel_id'] else "primary"):
                            st.session_state.selected_channel_id = channel['channel_id']
                            st.rerun()

            with msg_col2:
                if st.session_state.selected_channel_id:
                    channel_id_str = str(st.session_state.selected_channel_id)
                    user_id_str = str(st.session_state.current_user['user_id'])
                    ws_url = f"ws://127.0.0.1:8000/ws/{channel_id_str}/{user_id_str}"

                    # --- ШАГ 28: ИСПОЛЬЗОВАНИЕ WEBSOCKET КЛИЕНТА ---
                    # Загружаем историю один раз при выборе канала
                    if 'message_history' not in st.session_state or st.session_state.get('current_ws_channel') != channel_id_str:
                        headers = {"Authorization": f"Bearer {st.session_state.token}"}
                        messages_response = requests.get(f"http://127.0.0.1:8000/api/v1/channels/{channel_id_str}/messages", headers=headers)
                        if messages_response.status_code == 200:
                            st.session_state.message_history = messages_response.json()
                            st.session_state.current_ws_channel = channel_id_str
                        else:
                            st.session_state.message_history = []

                    # Запускаем клиент для получения новых сообщений
                    new_message_str = ws_client(send_requests=False, url=ws_url)
                    if new_message_str:
                        new_message = json.loads(new_message_str)
                        st.session_state.message_history.append(new_message)
                        st.rerun() # Перерисовываем чат с новым сообщением

                    # Отображение сообщений из истории
                    for msg in st.session_state.get('message_history', []):
                        # В реальном приложении нужно будет получать имена пользователей по sender_id
                        author_name = "User " + msg['sender_id'][:4]
                        st.chat_message("ai" if author_name == "AI" else "user").write(f"**{author_name}:** {msg['content']}")
                    ...
                    if prompt:
                        ...
                        # Отправляем сообщение через обычный POST-запрос, сервер сам разошлет его по WebSocket
                        requests.post(f"http://127.0.0.1:8000/api/v1/channels/{channel_id_str}/messages", headers=headers, json=payload)
                        # Не нужно делать rerun, новое сообщение придет по WebSocket
                    
                else:
                    st.info("← Выберите канал для просмотра сообщений.")
